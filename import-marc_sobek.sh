...
if [ $# -ne $EXPECTED_ARGS ]
then
  echo "    Usage: `basename $0` [-p ./path/to/import_sobek.properties] ./path/to/marc.mrc"
  exit $E_BADARGS
fi
...
then
  if [ -f "$VUFIND_LOCAL_DIR/import/import_sobek.properties" ]
  then
    PROPERTIES_FILE="$VUFIND_LOCAL_DIR/import/import_sobek.properties"
  else
    PROPERTIES_FILE="$VUFIND_HOME/import/import_sobek.properties"
  fi
fi
