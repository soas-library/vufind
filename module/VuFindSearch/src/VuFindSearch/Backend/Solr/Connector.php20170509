...
class Connector implements \Zend\Log\LoggerAwareInterface
{
    ...
    public function query($handler, ParamBag $params)
    {
        ...
        // In order to manage '&' character
        $paramString = str_replace('%26%26','##########',$paramString);
        //$paramString = str_replace('%26','%26%26',$paramString);
        $paramString = str_replace('Archive+%26%26+Special+Collections','Archive+%26+Special+Collections',$paramString);
        $paramString = str_replace('##########','%26%26',$paramString);
