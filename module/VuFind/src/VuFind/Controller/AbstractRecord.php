...
class AbstractRecord extends AbstractBase
{
    ...
    /**
     * Email action - Allows the email enquiry form to appear.
     *
     * @return \Zend\View\Model\ViewModel
     */
    public function emailenquiryAction()
    {
        // Force login if necessary:
        $config = $this->getConfig();
        if ((!isset($config->Mail->require_login) || $config->Mail->require_login)
        ...
        $mailer = $this->getServiceLocator()->get('VuFind\Mailer');
        $view = $this->createEmailViewModel(
            null, $mailer->getDefaultRecordEnquirySubject($driver)
        );
        $mailer->setMaxRecipients($view->maxRecipients);
        ...
        // Display the template:
        $view->setTemplate('record/emailenquiry');
        return $view;
    }
