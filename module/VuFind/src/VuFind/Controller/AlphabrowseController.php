...
class AlphabrowseController extends AbstractBase
{
	...
	//print_r('</br>');
        ...
        $view->total = $result['Browse']['totalCount']+($page*100);
	...
	$objPHPExcel->getProperties()
		->setCreator("SCB")
		->setLastModifiedBy("SCB")
		->setTitle($this->translate('Alphabrowse'))
		->setSubject($this->translate('Alphabrowse'))
		->setDescription($this->translate('Alphabrowse'))
		->setKeywords($this->translate('Alphabrowse'))
		->setCategory($this->translate('Alphabrowse'));


	$objSheet  = $objPHPExcel->getActiveSheet();
	
	$objSheet->getColumnDimension('A')->setWidth(50); //classmark
	$objSheet->getColumnDimension('B')->setWidth(100); //title
	$objSheet->getColumnDimension('C')->setWidth(100); //location
						
	$objSheet->getStyle('A1')->getFont()->setBold(true)->setName('Helvetica')->setSize(12)->getColor()->setRGB('000000');
	$objSheet->getStyle('B1')->getFont()->setBold(true)->setName('Helvetica')->setSize(12)->getColor()->setRGB('000000');
	$objSheet->getStyle('C1'.$contador)->getFont()->setBold(true)->setName('Helvetica')->setSize(12)->getColor()->setRGB('000000');
			
	$objSheet ->getCell('A1')->setValue($this->translate('CLASSMARK'));
	$objSheet ->getCell('B1')->setValue($this->translate('TITLE'));
	$objSheet ->getCell('C1')->setValue($this->translate('LOCATION'));	
		
		//Read array from alphabrowse
	        $cont=1;
	        foreach($arr_excel as $row){
	        	$cont=$cont+1;

			$objSheet ->getCell('A'.$cont)->setValue($this->translate($row[0])." \n");
			$objSheet ->getCell('B'.$cont)->setValue($this->translate($row[1])." \n");
			$objSheet ->getCell('C'.$cont)->setValue($this->translate($row[2])." \n");					
	        }
	        
	        $filename=$this->translate("Classmark");
	    
	    	if($from != "")
	        $filename=$this->translate($filename."_".$from);
	        
		$objPHPExcel->getActiveSheet()->setTitle($this->translate("Classmark"));
