...
class HierarchicalFacetHelper
{
    ...
    public function getExistingParent($parent,$ar_keys)
    {
    	$find=false;
    	foreach($ar_keys as $aux){
                	if($aux==$parent){$find=true;break;}
        }
	return $find;          
...
*/
        ...
        foreach ($keyedList as &$item) {
            ...
            if ($item['level'] > 0) {
              $arr_aux = array();
              $arr_exp=explode("/",$item['parent']);
              $arr_exp=array_filter($arr_exp, "strlen");
              $cont=count($arr_exp);
              $parent= $item['parent'];
             
             for($i=0;$i<$cont;$i++){

              	$find = $this->getExistingParent($parent,$ar_keys);
              	
              	if($find)$break;
              	else{
	        	array_pop($arr_exp);
	              	$arr_exp[0]=$arr_exp[0]-1;
        		$parent = implode("/",$arr_exp);
        		$parent = $parent."/";             	
        	}
              	
              }
              if($find)$keyedList[$parent]['children'][] = &$item;
