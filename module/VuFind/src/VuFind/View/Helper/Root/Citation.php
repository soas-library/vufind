...
class Citation extends \Zend\View\Helper\AbstractHelper
{
   ...
   /**
    ...
    // and is not followed by an edition statement:
        ...
        $harvard['periodAfterTitle'] = (!$this->isPunctuated($harvard['title']) && empty($harvard['edition']));
    ...
    // Behave differently for books vs. journals:
        ...
        if (empty($this->details['journal']))
           ...
           {
           list($harvard['volume'], $harvard['issue'], $harvard['date'])   = $this->getAPANumbersAndDate();
