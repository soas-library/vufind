...
class Mailer implements \VuFind\I18n\Translator\TranslatorAwareInterface
{
    ...
    /**
     * Send an enquiry email message representing a record.
     *
     * @param string                            $to      Recipient email address
     * @param string|\Zend\Mail\Address         $from    Sender name and email
     * address
     ...
     * @return void
     */
    public function sendEnquiryRecord($to, $from, $msg, $record, $view, $subject = null,
        $cc = null
    ) {
        if (null === $subject) {
            $subject = $this->getDefaultRecordEnquirySubject($record);
        }
        $body = $view->partial(
            'Email/record.phtml',
            [
    ...
    /**
     * Get the default subject line for sendEnquiryRecord()
     *
     * @param \VuFind\RecordDriver\AbstractBase $record Record being emailed
     *
     * @return string
     */
    public function getDefaultRecordEnquirySubject($record)
    {
          return $record->getEnquirySubject();
    }
