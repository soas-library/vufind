 ...
 * @link     https://vufind.org/wiki/configuration:external_content Wiki
 */
class CarouselLoader extends \VuFind\ImageLoader
{
    /**
    ...
    public function __construct($config, ApiManager $manager,
        ...
        $this->config = $config;
        $this->configuredFailImage = isset($config->Content->noCarouselCoverAvailableImage)
            ? $config->Content->noCarouselCoverAvailableImage : null;
            
       if (isset($this->config->Content->noCarouselCoverAvailableImageLeaves)) {
            $leaves = explode(',', $this->config->Content->noCarouselCoverAvailableImageLeaves);
            $claves_aleatorias = array_rand($leaves, 1);
            $this->configuredFailImage=$leaves[$claves_aleatorias];
        }
    ...
    public function getCoverGenerator()
    {
        ...
        $leaves = $this->config->Content->noCarouselCoverAvailableImageLeaves;
        return new \VuFind\Cover\CarouselGenerator($this->themeTools, $settings, $leaves);
    }
